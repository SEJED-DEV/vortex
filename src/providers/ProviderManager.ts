  ...
  private async callProvider(
    ...
    if (!key) {
      if (provider === 'gemini' || provider === 'groq') {
        console.warn(
          `[Config Warning] ${provider.toUpperCase()}_API_KEY is missing in .env! Skipping free-tier provider.`,
        );
      }
      return null;
    }
    ...
    try {
      switch (provider) {
        case 'gemini':
          return await this.callGemini(key, systemPrompt, history, userMessage, visionContent);
        case 'openai':
          return await this.callOpenAI(key, systemPrompt, history, userMessage, visionContent);
        case 'claude':
          return await this.callClaude(key, systemPrompt, history, userMessage, visionContent);
        case 'groq':
          return await this.callGroq(key, systemPrompt, history, userMessage);
        case 'mistral':
          return await this.callMistral(key, systemPrompt, history, userMessage);
        default:
          return null;
      }
    } catch (error: any) {
