...
const SecurityAudit: Skill = {
    ...
    execute: async (message: Message, data: any) => {
        ...
        const riskyEveryone = guild.roles.everyone.permissions.has(PermissionsBitField.Flags.MentionEveryone) || 
                             guild.roles.everyone.permissions.has(PermissionsBitField.Flags.ManageMessages);
        if (riskyEveryone) {
            report.push(`🚨 **@everyone Alert**: The default role has risky permissions (Mention Everyone or Manage Messages)!`);
        }

        const invitePermissions = guild.roles.cache.filter(r => r.permissions.has(PermissionsBitField.Flags.CreateInstantInvite));
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor('#FF0000')
            .setFooter({ text: 'Security scan requested by AI Manager' })
            .setTimestamp();
