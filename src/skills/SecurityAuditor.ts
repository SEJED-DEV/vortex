...
export const SecurityAuditor: Skill = {
  ...
  execute: async (message: Message, data: any): Promise<any> => {
    ...
    guild.roles.cache.forEach((role) => {
      ...
      if (isDangerous && role.rawPosition < guild.roles.cache.size / 2) {
        vulnerabilities.push(
          `⚠️ **Dangerous Role**: \`${role.name}\` has administrative permissions but is positioned low in the hierarchy.`,
        );
      }
    });
    ...
    guild.members.cache
      ...
      .forEach((bot) => {
        ...
        if (bot.permissions.has(PermissionFlagsBits.Administrator)) {
          vulnerabilities.push(
            `🤖 **Bot Admin**: <@${bot.id}> has \`Administrator\` permissions. Ensure this bot is trusted.`,
          );
        }
      });
    ...
    if (botMember && !botMember.permissions.has(PermissionFlagsBits.Administrator)) {
      vulnerabilities.push(
        `ℹ️ **Optimizer Note**: Vortex does not have \`Administrator\` permissions. Some advanced management actions may fail.`,
      );
    }
    if (guild.mfaLevel === 0) {
      vulnerabilities.push(
        `🔓 **2FA Disabled**: The server does NOT require Two-Factor Authentication for moderation actions. This is highly recommended to prevent nuke attacks.`,
      );
    }
    try {
      ...
      webhooks.forEach((webhook) => {
        ...
        if (channel && channel.type === ChannelType.GuildText) {
          ...
          if (everyonePermissions?.has(PermissionFlagsBits.ViewChannel)) {
            vulnerabilities.push(
              `🎣 **Exposed Webhook**: Webhook \`${webhook.name}\` exists in public channel <#${channel.id}>. If the URL is leaked, anyone can post as this webhook.`,
            );
          }
        }
      });
      ...
      vulnerabilities.push(
        `⚠️ **Scan Error**: Vortex lacks permission to scan Webhooks (Requires \`ManageWebhooks\`).`,
      );
    }
