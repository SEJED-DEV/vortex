...
const SentimentAnalysis: Skill = {
  ...
  description:
    'Analyzes the recent sentiment and mood of a channel or specific user messages to gauge community health',
  actionId: 'analyzeSentiment',
  jsonStructure:
    '{"action": "analyzeSentiment", "data": {"channelId": "CHANNEL_ID", "userId": "optional USER_ID", "limit": 50}}',
  execute: async (message: Message, data: any): Promise<string> => {
    ...
    let totalScore = 0;
    const breakdown: { [label: string]: number } = {
      'Very Positive': 0,
      Positive: 0,
      Neutral: 0,
      Negative: 0,
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: `${overall.emoji} Overall Mood`,
          value: `**${overall.label}** (Score: ${avg.toFixed(2)})`,
          inline: false,
        },
        {
          name: '📈 Breakdown',
