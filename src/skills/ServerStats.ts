...
const ServerStats: Skill = {
  name: 'Server Stats',
  description:
    'Generates a comprehensive server statistics report including member counts, channel breakdown, boost level, and activity overview',
  ...
  execute: async (message: Message, _data: any): Promise<string> => {
    ...
    const joinedThisWeek = guild.members.cache.filter(
      (m) => m.joinedTimestamp && now - m.joinedTimestamp < sevenDays,
    ).size;
