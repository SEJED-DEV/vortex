    ...
    execute: async (message: Message, data: any): Promise<string> => {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '📅 Account Created', value: `<t:${Math.floor(user.createdTimestamp / 1000)}:R> (${accountAge} days ago)`, inline: false },
                { name: '📥 Joined Server', value: member.joinedTimestamp ? `<t:${Math.floor(member.joinedTimestamp / 1000)}:R> (${joinAge} days ago)` : 'Unknown', inline: false },
